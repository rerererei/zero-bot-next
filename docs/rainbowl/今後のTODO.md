...
- [ ] **合格後フロー（了承済み→新人）に必要なDiscord側の手動権限設定**
  コード側（`acknowledged_role_id`・`newcomer_role_id`・`male_profile_channel_id`・`female_profile_channel_id`）は実装済み・DB投入済みだが、以下はDiscord側で手動設定が必要（未確認）：
  - 「了承済み」ロールに、規約・ルールカテゴリーの閲覧権限を「合格」ロールと同等以上に設定する
  - 「男性」ロールに男性プロフィールチャンネル（`1533732434087252049`）、「女性」ロールに女性プロフィールチャンネル（`1533732470443475034`）の閲覧権限を設定する（`@everyone`はデフォルト非表示にする）
  - 「新人」ロールに、一般利用可能なチャンネル群の閲覧権限を設定する（member限定チャンネルは除く）
...
- [x] **合格後フローを「合格→了承済み→新人」の3段階に再設計し、合格通知を本人専用チャンネルへ変更**（2026-09-09）
  設計変更：新設していた「仮会員」的ロールを「了承済み」（`acknowledged_role_id`、ID `1546942079299756082`）に位置づけ直し、「新人」ロール（既存ID `1546058079727259658`）は男性/女性プロフィールチャンネルへの初回投稿を検知した時点で付与するよう変更（[cogs/rainbowl_profile_gate.py](../../cogs/rainbowl_profile_gate.py)を新設）。ステータス遷移は`PASSED → ACKNOWLEDGED → NEWCOMER`（[data/rainbowl/applicants_store.py](../../data/rainbowl/applicants_store.py)の`set_acknowledged`/`set_newcomer`）。
  あわせて、合格通知を共有の「合格通知」チャンネルではなく本人専用チャンネルへ投稿する方式に変更（`passed_notice_channel_id`設定は廃止）。この変更により、本人専用チャンネルは`/ok`時点では削除されなくなり、了承済み〜新人になるまで案内チャンネルとして使い続ける設計になった。
  実装中に、既存コードの**バグ**を発見して修正：「了解しました」ボタンの処理内に本人専用チャンネル削除の呼び出しがあったが、`return True`の**後**に書かれていて実行されない死んだコードになっていた（結果として本人専用チャンネルが合格後も削除されず残り続けるバグだった）。今回の再設計で、削除処理を正しく「プロフィールチャンネルへの初回投稿検知」のタイミングに実装し直した。
  新規configフィールド（`acknowledged_role_id`・`male_profile_channel_id`・`female_profile_channel_id`）は、前回の教訓を踏まえて**コードデプロイ前に**`zero_bot_guild_config`へ値を投入済み。
  Discord側の権限設定（了承済みロールへの規約カテゴリー権限、男性/女性ロールへのプロフィールチャンネル権限、新人ロールへの一般チャンネル権限）はまだ未確認（上記「保留中」参照）。
